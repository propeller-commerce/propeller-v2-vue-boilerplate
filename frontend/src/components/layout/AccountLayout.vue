...
function isAuthManagerForCompany(): boolean {
  ...
  return contact?.roles?.includes('PURCHASE_AUTH_MANAGER') ||
    contact?.companyRoles?.some((r: any) => r.companyId === company.companyId && r.roles?.includes('PURCHASE_AUTH_MANAGER')) ||
    false;
}
