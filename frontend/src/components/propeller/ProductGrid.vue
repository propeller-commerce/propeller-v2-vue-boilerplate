<template>
  ...
  >
    <template v-if="getIsLoading()">
      <div :class="`propeller-product-grid__skeleton-grid ${getGridColsClass()}`">
        <template :key="idx" v-for="(_, idx) in getSkeletonItems()">
          ...
          >
            <div class="propeller-product-grid__skeleton-image aspect-square bg-muted animate-pulse"></div>
            <div class="p-4 flex flex-col gap-2 flex-1">
              <div class="propeller-product-grid__skeleton-line h-3 bg-muted animate-pulse rounded w-1/4"></div>
              <div class="propeller-product-grid__skeleton-line h-4 bg-muted animate-pulse rounded w-3/4"></div>
              <div class="propeller-product-grid__skeleton-line h-4 bg-muted animate-pulse rounded w-1/2"></div>
              <div class="mt-auto pt-2">
                <div class="propeller-product-grid__skeleton-line h-5 bg-muted animate-pulse rounded w-1/3"></div>
              </div>
            </div>
            <template v-if="showAddToCart()">
              <div class="p-4 pt-0">
                <div class="flex items-center gap-2">
                  <div class="propeller-product-grid__skeleton-line h-9 flex-1 bg-muted animate-pulse rounded"></div>
                  <div class="propeller-product-grid__skeleton-line h-9 flex-1 bg-muted animate-pulse rounded"></div>
                </div>
              </div>
            </template>
    ...
    <template v-if="!getIsLoading()">
      <template v-if="getDisplayProducts().length === 0">
        <div class="propeller-product-grid__empty text-center py-24 bg-muted rounded-xl border border-dashed border-border">
          <svg
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          ...
          <h3 class="propeller-product-grid__empty-title mt-4 text-lg font-semibold text-foreground">No products found</h3>
          <p class="propeller-product-grid__empty-message mt-1 text-sm text-muted-foreground">Try adjusting your filters or search term.</p>
        </div>
      </template>
...
<script setup lang="ts">
   ...
   import  { GraphQLClient, Product, Cluster, Cart, CartMainItem, AttributeFilter, ProductTextFilterInput, ProductsResponse, Category } from 'propeller-sdk-v2';
...
const {
  displayProducts, isLoading, itemsFound, currentSortField, currentSortOrder,
  currentPage, totalPages, fetchProducts, goToPage,
} = useProductSearch({
  graphqlClient: props.graphqlClient,
...
watch(() => isLoading.value, (v) => props.onLoadingChange?.(v));
