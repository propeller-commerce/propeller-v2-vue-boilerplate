...
- `PurchaseAuthorizationConfigurator.title` — EN: "Purchase Authorization Settings" → NL: "Autorisatie-instellingen" — Settled (PWP-1035): matches the account menu item beside it (`Account.navAuthorizationSettings`) and `Autorisatieaanvragen`. The "Inkoop" prefix was dropped so the heading and the menu do not disagree on one screen.
