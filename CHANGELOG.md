...
- **The last five reachable taxonomy events are wired — 46 of 47.**
  ...
  zero while counting only the removals the list-detail view emits.
  `propeller.favorite_list_created` / `_updated` / `_deleted` and
...
- **The set-default address event was thrown away.**
  `AddressesView.handleSetDefault` emitted `propeller.address_default_changed`,
  a name the taxonomy does not contain, and the ingest discards unrecognised
  names without an error — so the metric read zero since tracking shipped. The
  correct name is `propeller.address_set_default`, which propeller-next emits.
- **Unknown event names now fail to compile.** `src/lib/tracking/bus.ts` took
  `name: string`, which erased the taxonomy check for every call site behind
  the facade — that is why the typo above survived review and `vue-tsc`. It
  takes `EventName` now. The shared `tracker.ts` core keeps `string`: it is
...
- **The search dropdown and the quick-order typeahead found nothing on a
  ...
  the catalog root, which is the intended default. Package
...
- **The catalog root is now channel-driven on the client too**.
...
- **`/api/graphql` and `/api/order-editor` are rate limited.** Both proxies
  ...
  refinement requested on the Next side, applied here from the start.
  ...
  the upstream starts seeing pathological nesting.
...
- `@propeller-commerce/propeller-v2-vue-ui` **0.13.0 → 0.14.0** — keyboard and
  screen-reader operable checkout selectors and language-aware
  product names, which is what put German names on a Dutch checkout review and
  thank-you page. Carries core-ui 0.6.1.
