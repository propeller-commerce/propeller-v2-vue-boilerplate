<template>
  <div :class="className || ''">
    <button
      type="button"
      class="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors"
      @click="
        async (event) => {
          showModal = true;
        }
    ...
    <template v-if="showModal">
      <div
        class="fixed inset-0 bg-black/50 flex items-start justify-center z-50 overflow-y-auto py-10"
      >
        <div class="bg-white p-6 rounded-lg max-w-2xl w-full mx-4 shadow-xl">
          ...
          <template v-if="getAddresses().length > 0">
            <div class="grid grid-cols-2 gap-4">
              <template :key="address.id" v-for="(address, index) in getAddresses()">
                <div
                  ...
                  :class="`cursor-pointer rounded-lg transition-all ring-2 ${
            ...
            <div class="flex justify-end mt-6 pt-4 border-t border-gray-100">
              <button
                type="button"
                class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-white rounded-md text-sm font-medium hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
